...
local group_blocks, blocks, fixed_blocks = nil, nil, {}
...
local updateHeroTalent, readKeybindings, updateSpellOverride, updateSpellIcon, updateSpellCooldownByEvent
...
function frame:UNIT_SPELLCAST_SUCCEEDED(unitTarget, castGUID, spellID, castBarID)
    if not issecretvalue(spellID) then
        if spellID == 384255 and type(updateHeroTalent) == "function" then
            C_Timer.After(1, function() updateHeroTalent() end)
        end
        if type(updateSpellSuccess) == "function" then updateSpellSuccess(spellID) end
    end
end
...
function frame:SPELL_ACTIVATION_OVERLAY_SHOW(spellId)
    if type(spellActivationOverlayShow) == "function" then
        spellActivationOverlayShow(spellId)
    end
end

function frame:SPELL_ACTIVATION_OVERLAY_HIDE(spellId)
    if type(spellActivationOverlayHide) == "function" then
...
function frame:COOLDOWN_VIEWER_SPELL_OVERRIDE_UPDATED(baseSpellID, overrideSpellID)
    if type(updateSpellOverride) == "function" then
        updateSpellOverride(baseSpellID, overrideSpellID)
    end
end

frame:RegisterEvent("SPELL_UPDATE_ICON") -- 法术图标更新
function frame:SPELL_UPDATE_ICON(spellId)
    if type(updateSpellIcon) == "function" and spellId then updateSpellIcon(spellId) end
end
